...
def format_ff_response(data):
    ...
    if isinstance(data, dict):
        formatted_response = text(
            ...
            bold("🏷 Вид Маркировки:") + f" {escape_md(data['marking_type_title'])}",
            bold("✅ Честный знак:") + f" {'Да' if data['honest_sign'] else 'Нет'}",
            bold("📦 Упаковка:") + f" {escape_md(data['package_title'])}",
            bold("📏 Вид Упаковки:") + f" {escape_md(data['packaging_size'])}",
            bold("🏷 Биркование:") + f" {'Да' if data['need_taging'] else 'Нет'}",
            bold("📎 Вложения:") + f" {'Да' if data['need_attachment'] else 'Нет'}",
