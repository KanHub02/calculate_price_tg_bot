...
async def send_pdf(callback_query: types.CallbackQuery, callback_data: dict):
    ...
    try:
        await bot.send_message(callback_query.from_user.id, "⏳ Пожалуйста, подождите...")

        await download_file(pdf_url, local_file_path)

        await bot.send_document(callback_query.from_user.id, types.InputFile(local_file_path))

        if os.path.exists(local_file_path):
            os.remove(local_file_path)

    except Exception as e:
        await bot.send_message(callback_query.from_user.id, "Ошибка при обработке файла.")
