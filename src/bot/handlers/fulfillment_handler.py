...
async def set_quantity(message: types.Message, state: FSMContext):
    ...
    if not is_float(message.text):
        new_message = await message.reply(
            "Пожалуйста, введите правильное числовое количество."
        )
        await state.update_data(last_message_id=new_message.message_id)
        return
...
async def set_need_defect_check(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    new_message = await bot.send_message(
        callback_query.from_user.id, "Нужна ли маркировка?", reply_markup=answerkb()
    )
    await state.update_data(last_message_id=new_message.message_id)
    await FulfillmentForm.ask_marking_type.set()
...
async def ask_marking_type(message: types.Message, state: FSMContext):
    ...
    if message.text.lower() == "да":
        marking_types, error = await fetch_and_check(
            fetch_marking_types,
            "Типы маркировки не найдены, пожалуйста, попробуйте позже.",
        )
        if error:
            await bot.send_message(
                message.from_user.id, error, reply_markup=main_menu_keyboard()
...
async def ask_packaging(message: types.Message, state: FSMContext):
    ...
    if message.text.lower() == "да":
        ...
        new_message = await bot.send_message(
            message.from_user.id, "Наличие своих материалов?", reply_markup=answerkb()
        )
        await state.update_data(last_message_id=new_message.message_id)
        await FulfillmentForm.ask_material.set()
    else:
        new_message = await message.reply(
            "Нужна ли упаковка? Введите Да или Нет", reply_markup=answerkb()
        )
        await state.update_data(last_message_id=new_message.message_id)
        return
...
async def ask_material(message: types.Message, state: FSMContext):
    ...
    new_message = await bot.send_message(
        message.from_user.id, "Нужно ли биркование?", reply_markup=answerkb()
    )
    await state.update_data(last_message_id=new_message.message_id)
...
async def set_tagging(message: types.Message, state: FSMContext):
    ...
    if message.text.lower() == "да":
        ...
        new_message = await message.reply(
            "Нужно ли биркование? Введите Да или Нет", reply_markup=answerkb()
        )
        await state.update_data(last_message_id=new_message.message_id)
        return
    ...
    new_message = await bot.send_message(
        message.from_user.id, "Нужны ли вложения?", reply_markup=answerkb()
    )
    await state.update_data(last_message_id=new_message.message_id)
...
async def set_inserts(message: types.Message, state: FSMContext):
    ...
    if message.text.lower() == "да":
        ...
        new_message = await message.reply(
            "Нужны ли вложения? Введите Да или Нет", reply_markup=answerkb()
        )
        await state.update_data(last_message_id=new_message.message_id)
        return
    ...
    new_message = await bot.send_message(
        message.from_user.id,
        "Введите количество товаров в коробке 60x40x40:",
        reply_markup=answerkb(),
    )
    await state.update_data(last_message_id=new_message.message_id)
...
async def set_warehouse(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(
        callback_query.from_user.id, "Выберите действие:", reply_markup=keyboard
    )
    await state.finish()
